...
/**
        ...
        public static String getFileAsString(String filePath) throws IOException {
            ...
            try (Stream<String> stream = Files.lines(Paths.get(filePath))) {
                ...
                stream.forEach(line -> message.append(line).append(newline));
         ...
         * @param expiration of the authentication request in milliseconds; cannot be longer than the value defined
         *                   on the pod
         * @param privateKey the private RSA key to be used to sign the authentication request; will be checked on
         *                   the pod against
         ...
         */
        private static PrivateKey parseRSAPrivateKey(String privateKeyFilePath)
            ...
            try {
                ...
                if (pemPrivateKey.contains(PEM_PRIVATE_START)) {              // PKCS#8 format
                    ...
                    String privateKeyString = pemPrivateKey.replace(PEM_PRIVATE_START, "").replace(PEM_PRIVATE_END, "")
                            .replace("\\n", "\n").replaceAll("\\s", "");
                    ...
                    try (PemReader pemReader = new PemReader(new StringReader(pemPrivateKey))) {
                        ...
                        RSAPrivateCrtKeyParameters privateKeyParameter = new RSAPrivateCrtKeyParameters(rsa.getModulus(),
                                rsa.getPublicExponent(), rsa.getPrivateExponent(), rsa.getPrime1(), rsa.getPrime2(), rsa.getExponent1(),
